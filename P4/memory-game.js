...
const generateGame = () => {
    ...
    // Restablecer el contenido del display de movimientos y tiempo
    selectors.movimientos.innerText = `0 movimientos`;
    selectors.timer.innerText = `tiempo: 0 sec`;
    selectors.comenzar.disabled = false;

    clearInterval(state.loop);
    const dimensions = parseInt(selectors.gridSizeSelect.value);
...
const resetGame = () => {
    ...
    // Reiniciamos el display de movimientos y tiempo
    selectors.movimientos.innerText = '0 movimientos';
    selectors.timer.innerText = 'tiempo: 0 sec';

    // Reiniciamos el mensaje de victoria
    selectors.win.innerHTML = '';

    // Habilitamos el botón de comenzar
    selectors.comenzar.classList.remove('disabled');

    // Restablecer el estado de todas las cartas, quitando las clases de 'flipped' y 'matched'
    document.querySelectorAll('.card').forEach(card => {
        card.classList.remove('flipped', 'matched');
    });
...
const startGame = () => {
    // Reiniciamos el tiempo a cero y otros contadores
    state.totalTime = 0;
    state.totalFlips = 0;
    selectors.movimientos.innerText = `0 movimientos`;
    selectors.timer.innerText = `tiempo: 0 sec`;
...
// Asignamos el manejador de eventos para el botón de restablecer
document.getElementById('reiniciar').addEventListener('click', function() {
    resetGame();
});

    // Vuelve a generar el juego
    generateGame();
    })
